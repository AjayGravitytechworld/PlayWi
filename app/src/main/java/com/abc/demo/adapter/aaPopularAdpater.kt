...
class aaPopularAdpater(
    private val context: Context,
    private var trending_iteams: ArrayList<aaVideoModel>
) : RecyclerView.Adapter<aaPopularAdpater.Myvh>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Myvh {
        val view: View =
            LayoutInflater.from(parent.context).inflate(R.layout.popularvideo_item, parent, false)
    ...
    override fun onBindViewHolder(holder: Myvh, position: Int) {
        ...
        holder.image.setOnClickListener {
            ...
            AppManage.getInstance(context).showInterstitialAd(
                context,
                {
                    db!!.addContact(aaIncome("Popular Watch Video", "20"))
                    store!!.putInt(
                        Utilll.str_todayCoin,
                        store!!.getInt(Utilll.str_todayCoin) + 20
                    )
                    store!!.putInt(
                        Utilll.str_totalCoin,
                        store!!.getInt(Utilll.str_totalCoin) + 20
                    )
    ...
    override fun getItemCount(): Int {
        return if (trending_iteams.size > 20)
            20
        else
            trending_iteams.size
    }
